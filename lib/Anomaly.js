...
class Anomaly extends EventEmitter {
     ...
     * @param {object} system the system being analyzed
     * @param {object} opts arbitrary options
     * @throws {Error} error if the anomaly is not in a preaction state
    ...
    async revert(system, opts) {
        ...
        if(this.#use_fingerprints) {
            ...
            if(this.state == "preaction") {

                this.log.info(`Anomaly is in a preaction state, verifying fingerprint`);
                let fingerprint = this.fingerprint(system);
                if(fingerprint != this.#fingerprints.preaction) { 

                    throw new Error(`Fingerprint mismatch: ${fingerprint} != ${this.#fingerprints.preaction}`);

                } else {

                    this.log.info(`Fingerprint matches, no need to revert`);
                }

                this.emit("activity", { anomaly: this, activity: "revert", progress: 100 });
                return;
            }

            // We need to perform a reversion.  Take the fingerprint before we revert, make sure it matches the postaction fingerprint
            let fingerprint = this.fingerprint(system);
            ...
            if(this.state == "preaction") {
                ...
                this.log.info(`Anomaly is in a preaction state, no need to revert`);
                this.emit("activity", { anomaly: this, activity: "revert", progress: 100 });
                return;
            }
            await this._revert(system, opts);
    ...
    async evaluate(system, opts) {
        ...
        await this._anomaly.evaluate(system, opts);
