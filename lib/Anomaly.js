...
class Anomaly extends EventEmitter {
     ...
     * 
     * @param {object} system the system being analyzed
     * @param {array<string>} [only_keys] only use these keys.  By default, uses all keys that don't point to functions. Use full paths, e.g. ["a.name", "b.name.first"]
     * @returns {string} the fingerprint
     * @throws {Error} error if we can't find a specified key
     */
    fingerprint(system, only_keys = null) {

        // Get all keys from the system in alphabetical order (for consistency, since object key order is not guaranteed)
        let keys = KeyExtractor.extract(system).filter((key)=>key.type != "function" ).sort(Key.compare);

        // Filter out keys that are not in the specified list (if any)
        if(only_keys !== null) { 

            let only = [...only_keys];  // Copy so we can modify
